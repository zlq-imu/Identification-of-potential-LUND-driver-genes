...
parser <- add_argument(parser, "--forest",help="forest for significant genes through univariate cox analysis")
...
parameters <- parse_args(parser,c("--expr_file","E:/桌面/Github/raw_data/LUAD_exp_file(TPM).txt",
                            ...
                            "--ROCplot","ROCplot",
                            "--forest","uni_cox_forest_for_each_gene",
                            "--PHtest","PHtest",
...
##Forest plot for significant genes ====================================

gene <- rownames(Siggene)
hr <- sprintf("%.3f",Siggene$"HR")
hrLow  <- sprintf("%.3f",Siggene$"Low.95.CI")
hrHigh <- sprintf("%.3f",Siggene$"High.95.CI")
Hazard.ratio <- paste0(hr,"(",hrLow,"-",hrHigh,")")
pVal<-format(signif(Siggene$P_value,4),scientific = TRUE)

##Graphics output=======================================================

pdf(paste(parameters$pdf,parameters$forest,".pdf",sep = ""),onefile = FALSE, width=7,height=6)
n <- nrow(Siggene)
nRow <- n+1
ylim <- c(1,nRow)
...
boxcolor = ifelse(as.numeric(hr) > 1, "#377EB8", "#377EB8")
